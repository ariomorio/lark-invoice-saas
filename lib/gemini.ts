...
1. **明細行の抽出**:
   ...
   - 表の「ヘッダー行」と「合計行」は除外し、データ行のみを抽出してください
   - 各データ行から以下を抽出：
     * description: 品目名・内容（例: "ゆいみちこ 様 (2025年12月分)"）
     * quantity: 数量（数値）
     * unitPrice: 税抜単価（数値）
     * amount: 税抜金額（数値、unitPrice × quantity）

2. **金額の扱い**:
   - 金額から「¥」「,」などの記号を除去し、数値のみを抽出
   - 単価（unitPrice）は必ず「税抜金額」として抽出
   - 入力が「税込」の場合は、1.1で割って税抜金額を算出
   - **マイナス金額も対応**: 「インボイス制度による調整（2％）-12,000円」のような場合、amountを負の数（-12000）として抽出

3. **日付の形式**:
   - 日付は必ずYYYY-MM-DD形式に変換（例: 2025-12-31）
   - 「2025年12月31日」→「2025-12-31」

4. **その他**:
   - 発行者情報（issuer）は空文字列のまま
   - 情報がない項目は空文字列または0

【表データ抽出の例】
入力:
| 品目 / 内容 | 単価 | 数量 | 金額 (税込) |
| ゆいみちこ 様 (2025年12月分) | ¥14,000 | 1 | ¥14,000 |
| まえのえみ 様 (2025年12月分) | ¥84,000 | 1 | ¥84,000 |

出力:
"items": [
  {
    "description": "ゆいみちこ 様 (2025年12月分)",
    "quantity": 1,
    "unitPrice": 12727,
    "amount": 12727
  },
  {
    "description": "まえのえみ 様 (2025年12月分)",
    "quantity": 1,
    "unitPrice": 76364,
    "amount": 76364
  }
...
【出力JSON形式】
{
  ...
  "recipient": {
    "name": "株式会社Levela",
    "address": "東京都渋谷区宇田川町42-6TQ渋谷宇田川町3階",
    "postalCode": "1500042"
  },
  "issuer": {
    "name": "",
...
【入力テキスト】
以下のテキストから上記ルールに従って情報を抽出してください。
特に表形式のデータがある場合は、各データ行を個別の明細として抽出してください。
