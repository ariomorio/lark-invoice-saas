...
export async function POST(request: NextRequest) {
    try {
        try {
            ...
            console.log('Webhook raw body:', rawBody); // Debug log
            ...
            // typeプロパティが存在し、かつ 'url_verification' である場合
            if ('type' in body && body.type === 'url_verification') {
                console.log('Sending challenge response:', body.challenge);
                return NextResponse.json({
            ...
            // encryptプロパティがある場合
            if ('encrypt' in body) {
                console.error('Received encrypted event. Please disable encryption in Lark Console.');
                return NextResponse.json(
